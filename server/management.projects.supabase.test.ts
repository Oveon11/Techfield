...
describe("management.projects.list with Supabase", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("utilise le helper Supabase et conserve le contrat attendu des chantiers", async () => {
    ...
    mocks.getSupabaseProjectsList.mockResolvedValue([
      {
        id: 12,
        reference: "CH-ABCDE123",
        title: "Rénovation chaufferie",
        status: "en_cours",
        progressPercent: 55,
        clientName: "OVEON",
        siteName: "Siège Paris",
        assignedTechnicians: "Marc Martin, Léa Durand",
      },
    ]);
    ...
    const result = await caller.management.projects.list();

    expect(mocks.getSupabaseProjectsList).toHaveBeenCalledWith(expect.objectContaining({
