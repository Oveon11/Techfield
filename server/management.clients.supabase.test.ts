...
describe("management.clients.list with Supabase", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("utilise le helper Supabase et conserve le contrat attendu des clients", async () => {
    ...
    mocks.getSupabaseClientsList.mockResolvedValue([
      {
        id: 5,
        companyName: "OVEON",
        legalName: "OVEON SAS",
    ...
    const result = await caller.management.clients.list();

    expect(mocks.getSupabaseClientsList).toHaveBeenCalledWith(expect.objectContaining({
